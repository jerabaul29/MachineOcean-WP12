...
import datetime
import numpy as np
from motools.storm_surge.kartverket.api import KartverketAPI, AccessStormSurgeNetCDF, NetCDFTester
...
def test_correctness_netcdf_generation_extraction():
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        print("for an unknown reason, this API request has stopped to work and return only an empty dataset now, which makes this test fail.")
...
def test_correctness_netcdf_generation_extraction_long_test_at_random():
    with tempfile.TemporaryDirectory() as tmpdirname:
        kartverket_api = KartverketAPI(short_test=False, cache_folder=tmpdirname)

        date_start = datetime.date(2009, 12, 8)
        date_end = datetime.date(2010, 1, 25)
        ...
        tester.perform_random_tests(n_tests=10000)
